---
title: "Le filtre de Kalman"
author: "Pierre Gloaguen"
date: "30 janvier 2020"
output:
  pdf_document:
    toc: yes
    number_sections: yes
  html_document:
    number_sections: yes
    toc: yes
    toc_float: yes
header-includes: \renewcommand{\contentsname}{}
---

```{r setup, include=FALSE, message = FALSE}
library(tidyverse)
library(mvtnorm)
knitr::opts_chunk$set(echo = F)
```

# Modèle générique en en temps discret

On suppose qu'on observe un processus stochastique $\left\lbrace Y_t \right\rbrace_{t \geq 0}$ à valeurs dans $\mathbb{R}^{d_Y}$ de la manière suivante (on se focalise tout d'abord sur un temps discret):

$$
\left\lbrace
\begin{array}{rcl}
X_0  &=& \mu_0 + B_0 \varepsilon_0\\
X_{t} &=& \mu_t + A_t X_{t - 1} + B_t \varepsilon_t,~t\geq 1\\
Y_t &=& C_t X_t + D_t \tilde{\varepsilon}_t
\end{array}
\right.
$$

où 

- $\left\lbrace X_t \right\rbrace_{t \geq 0}$ est un processus stochastique à valeurs dans $\mathbb{R}^{d_X}$ qui n'est pas observé.
- $\left\lbrace \mu_t\right\rbrace_{t\geq 0}$ est un ensemble de vecteurs de dimension $d_X$,
- $\left\lbrace \varepsilon_t\right\rbrace_{t\geq 0}$ désigne un ensemble de vecteurs aléatoires indépendants, de loi Gaussienne standard (à composantes indépendantes, et de variance 1) de dimension $d_\varepsilon$. 
- $\left\lbrace A_t \right\rbrace_{t\geq 0}$ est un ensemble de matrices de dimensions $d_X \times d_X$, décrivant la dynamique (linéaire, donc) du processus caché.
- $\left\lbrace B_t \right\rbrace_{t\geq 0}$ est un ensemble de matrices de dimensions $d_X \times d_\varepsilon$, décrivant la variance de l'innovation du processus caché.
- $\left\lbrace C_t \right\rbrace_{t\geq 0}$ est un ensemble de matrices de dimensions $d_Y \times d_X$, décrivant le modèle d'observation.
- $\left\lbrace \tilde{\varepsilon}_t\right\rbrace_{t\geq 0}$ désigne un ensemble de vecteurs aléatoires indépendants, de loi Gaussienne standard (à composantes indépendantes, et de variance 1) de dimension $d_{\tilde{\varepsilon}}$. 
- $\left\lbrace D_t \right\rbrace_{t\geq 0}$ est un ensemble de matrices de dimensions $d_Y \times d_{\tilde{\varepsilon}}$, décrivant la variance du modèle d'observation.

# Exemple en écologie

## Modèle de déplacement imparfaitement observé en écologie du mouvement.

On se place dans le cadre où on dispose d'observations de positions d'un animal obtenues grâce à des capteurs GPS à différents temps $t_0 = 0, t_1 = 1,\dots, t_n = n$.
On note $Y_0, \dots, Y_n$ les positions observées, qui sont donc dans $\mathbb{R}^2$ ($d_Y = 2$). 
On suppose que le GPS est entâchée d'une erreur aléatoire, distribuée selon une loi normale bivariée, centrée et de variance $\sigma^2$.

Dans ce cas, le processus caché est donnée par la vraie position ainsi que par la vitesse de l'animal.
On suppose que la dynamique de la vitesse bivariée (notée $V_t = (V_{t, 1}, V_{t, 2})'$ ) est linéaire et Gaussienne, c'est à dire que la vitesse à l'instant $t$ est une variable aléatoire Gaussienne, centrée en une application linéaire de la vitesse à l'instant $t - 1$ (plus un éventuel terme constant). 
La vraie position à l'instant $t$ (notée $Z_t = (Z_{t, 1}, Z_{t, 2})')$ est elle obtenue par la position à l'instant $t - 1$ plus le déplacement induit par la vitesse $V_{t - 1}$.

Pour résumé, de manière générique, le processus caché $X_t$ est le vecteur:
$$
X_t = 
\begin{pmatrix}
V_{t, 1}\\
V_{t, 2}\\
Z_{t, 1}\\
Z_{t, 2}
\end{pmatrix}.
$$

On suppose que $\mu_t$ et $A_t$ sont de la forme:

$$
\mu_t = 
\begin{pmatrix}
c_1(t) \\
c_2(t) \\
0 \\
0
\end{pmatrix}
A_t = 
\begin{pmatrix}
\alpha_1(t) & \beta_1(t) & 0 & 0\\
\beta_2(t) & \alpha_2(t) & 0 & 0 \\
1 & 0 & 1 & 0\\
0 & 1 & 0 & 1
\end{pmatrix},
$$

où les $c_1(t), c_2(t), \alpha_1(t), \alpha_2(t) \beta_1(t), \beta_2(t)$ sont des fonctions décrivant la dynamique du processus\footnote{Par exemple, si $\left\lbrace V_{t, 1} \right\rbrace_{t\geq 0}$ suit un $AR(1)$, indépendant $\left\lbrace V_{t, 2} \right\rbrace_{t\geq 0}$, de paramètres $\mu$ et $\rho$, alors $c_1(t) = \mu$, $\alpha_1(t) = \rho$ et $\beta_1(t)  = 0$.}. 

Dans ce processus caché, l'aléa provient du seul processus de vitesse, ainsi $d_\varepsilon = 2$ et la matrice $B(t)$ s'écrit:

$$
B_t = 
\begin{pmatrix}
\sigma_{11}(t) & \sigma_{12}(t) & \sigma_{13}(t) & \sigma_{14}(t)\\
\sigma_{21}(t) & \sigma_{22}(t) & \sigma_{23}(t) & \sigma_{24}(t)\\
\sigma_{31}(t) & \sigma_{32}(t) & \sigma_{33}(t) & \sigma_{34}(t)\\
\sigma_{41}(t) & \sigma_{42}(t) & \sigma_{43}(t) & \sigma_{44}(t)
\end{pmatrix}
$$

Concernant le processus d'observation, les matrices $C_t$ et $D_t$  sont simplement:

$$
C_t = \begin{pmatrix}
0 & 0 & 1 & 0\\
0 & 0 & 0 & 1
\end{pmatrix},~
D_t = \begin{pmatrix}
\sigma & 0\\
0 & \sigma
\end{pmatrix}
$$


## Exemple paramétré

On reprend l'exemple précédent avec les fonctions suivantes\footnote{Le choix des valeurs n'est pas anodin et correspond à un certain paramétrage de deux processus de Ornstein Uhlenbenck indépendants.} pour les matrice $A_t$ et $B_t$:

$$
\left\lbrace 
\begin{array}{rl}
c_1(t)      &= 0\\
\alpha_1(t) = \alpha_2(t) &= \text{e}^{-1}\\
c_2(t)      &= (1 - \text{e}^{-1})\\
\alpha_2(t) &= \text{e}^{-1}\\
\beta_1(t)  = \beta_2(t)  &= 0\\
\sigma_{11}(t)  = \sigma_{22}(t)  &= \sqrt{2}\\
\sigma_{33}(t)  = \sigma_{44}(t)  &= \sqrt{2}\\
\sigma_{12}(t)  = \sigma_{12}(t)  &= 0
\end{array}
\right.
$$

On prend de plus, pour le bruit d'observation, $\sigma = 0.1$.

```{r parameters_list}
get_partial_iou_parameters <- function(rho, mu, sigma, sigma_obs, dt = 0.01){
  # Recurrent terms
  e_rt <- exp(-rho * dt)
  e_rt_sq <- e_rt^2
  recall  <- 1 - e_rt
  sig2 <- sigma^2
  var_asympt <- 0.5 * sig2 / rho
  var_v <- var_asympt * (1 - e_rt_sq)
  cov_v_z <- var_asympt * (1 - e_rt)^2 / rho
  var_z <- var_asympt / rho * (2 * dt + (-e_rt_sq + 4 * e_rt - 3) / rho)
  m0 <- c(mu, 0, 0)
  m <- c(recall * mu,  (dt - recall / rho)* mu)
  A <- matrix(c(e_rt * 
                  c(1, 0, 0, 0,
                    0, 1, 0, 0),
                recall / rho, 0         , 1, 0,
                0           , recall/rho, 0, 1),
              nrow = 4, ncol = 4, byrow = T)
  B <- matrix(c(var_v  , 0      , cov_v_z, 0      ,
                0      , var_v  , 0      , cov_v_z,
                cov_v_z, 0      , var_z  , 0      ,
                0      , cov_v_z, 0      , var_z),
              nrow = 4, ncol = 4, byrow = T)
  C = matrix(c(0, 0, 1, 0,
               0, 0, 0, 1),
             nrow = 2, ncol = 4, byrow = T)
  D = sigma_obs * diag(1, 2)
  list(dt = dt, m0 = m0, m = m, A = A, B = B, C = C, D = D)
}
```

```{r simulation_processus}
rpiou <- function(n, parametre){
  n <- ceiling(n) # On s'assure que ce soit un entier
  if(n < 1){
    stop("n should be an integer at least equal to 1")
  }
  dim_x <- 4
  dim_y <- 2
  x <- matrix(NA, nrow = dim_x, ncol = n + 1)
  y <- matrix(NA, nrow = dim_y, ncol = n + 1)
  x[, 1] <- parametre$m0 + rmvnorm(1, sigma = parametre$B) # Etat initial
  y[, 1] <- parametre$C %*% x[, 1] + t(rmvnorm(1, sigma = parametre$D))
  for(k in 1:n){
    x[, k + 1] <- parametre$m + parametre$A %*% x[, k] + t(rmvnorm(1, sigma = parametre$B))
    y[, k + 1] <- parametre$C %*% x[, k + 1] + t(rmvnorm(1, sigma = parametre$D))
  }
tibble(t = seq(0, by = parametre$dt, length.out = n + 1), 
         y1 = y[1, ], y2 = y[2, ], 
         v1 = x[1, ], v2 = x[2, ],
         z1 = x[3, ], z2 = x[4, ])
}
```


```{r premiere_simu, echo = -1}
set.seed(333)
par_simu <- get_partial_iou_parameters(rho = 0.1, mu = c(0, 0), 
                                       sigma = sqrt(2), sigma_obs = 1,
                                       dt = 1)
simulation <- rpiou(n = 2000, parametre = par_simu)
```

```{r representation}
simulation %>% 
  ggplot() +
  geom_path(aes(x = y1, y = y2, color = t)) + 
  geom_point(aes(x = y1, y = y2, color = t)) +
  geom_path(aes(x = z1, y = z2), color = "red") +
  scale_color_gradient(low = "yellow", high = "blue") +
  labs(x = expression(Y[1]),
       y = expression(Y[2]),
       color = "Temps")
```

## Objectif du filtre de Kalman {#objectif}

Le but du filtre de Kalman est, à partir des observations (les points sur la figure précédente) et du modèle posé, de retrouver les états cachés (la ligne rouge sur la figure précédente).
Pour cela, notre objectif est de connaître *la loi* des états cachés **sachant les observations**, à savoir:
la loi de $X_t \vert Y_{0:n}$.

# Filtrage et lissage de Kalman

## Remarques de notations

Dans la suite, on supposera que toutes les variables aléatoires considérées admettent une densité par rapport à la mesure de Lebesgue.

De manière abusive, la densité d'une variable aléatoire sera toujours notée comme une fonction $p()$.
Cette notation est surchargée, au sens où la référence à la variable aléatoire est contenue dans ses arguments.

Ainsi, $p(y_k)$ consiste en la densité de la variable aléatoire $Y_k$, évaluée en $y_k \in \mathbb{R}^{d_Y}$.
De même $p(x_k \vert y_k)$ est la densité de la variable aléatoire $X_k\vert Y_k = y_k$, évaluée en $x_k \in \mathbb{R}^{d_X}$.

De même, quand l'intervalle d'intégration n'est pas précisé, il s'agit alors d'une intégration sur tout le support de la loi.

## Détermination des quantités nécessaires

Notre objectif est de connaître la loi de $X_k \vert \left\lbrace Y_{0:n} = y_{0:n} \right\rbrace$.
Cette variable aléatoire admet la densité $p(x_k \vert y_{0:n})$.
Par conditionnement et loi des probabilités totales, on a, pour $0\leq k \leq n - 1$:

\begin{align*}
p(x_k \vert y_{0:n}) &= \int p(x_k, x_{k + 1} \vert y_{0:n}) \text{d} x_{k + 1} & \text{Probabilités totales}\\
&= \int p(x_{k} \vert x_{k +1},  y_{0:n}) p(x_{k + 1} \vert y_{0:n}) \text{d} x_{k + 1} & \text{Conditionnement}\\
&= \int p(x_{k} \vert x_{k +1},  y_{0:k}) p(x_{k + 1} \vert y_{0:n}) \text{d} x_{k + 1} & \text{Propriétés d'indépendance conditionnelle du modèle}\\
&\propto \int p(x_{k}, x_{k +1}\vert  y_{0:k}) p(x_{k + 1} \vert y_{0:n}) \text{d} x_{k + 1} & \text{La constante ne dépend pas de $x_k$}\\
&\propto \int p(x_{k + 1}\vert x_{k})p(x_k \vert  y_{0:k}) p(x_{k + 1} \vert y_{0:n}) \text{d} x_{k + 1} & \text{Propriétés du modèle}
\end{align*}

La propriété remarquable du modèle générique proposé ici est que le résultat de cet intégrale est calculable explicitement.
La loi résultante est celle d'une loi Gaussienne dont la moyenne et la matrice de variance covariance sont explicites.

## Propriétés de conjugaison de la loi normale multivariée

Soient deux variables aléatoires $U$ et $V$ telles que 
$$U \sim \mathcal{N}(m_U, \Sigma_U),~V\vert U = u \sim \mathcal{N}(Au + b,\Sigma_{V\vert U})$$
alors,
\begin{align}
U \vert \left\lbrace V = v \right\rbrace &\sim \mathcal{N}\left(\Sigma_{U\vert V}\left(A'\Sigma_{V\vert U}^{-1}(v - b) + \Sigma_U^{-1}m_U \right), \Sigma_{U\vert V}\right) \label{eq:loi:U:cond:V},
\end{align}
où:
\begin{align}
\Sigma_{U\vert V} &= \left(\Sigma_{U}^{-1} + A'\Sigma_{V\vert U}^{-1}A\right)^{-1} \label{eq:var:U:cond:V} \\
V &\sim \mathcal{N}\left(A m_U + b,  \Sigma_{V\vert U} + A \Sigma_U A'\right) \label{eq:loi:V}
\end{align}

En utilisant l'équation (C.7) du livre de Bishop, on peut réécrire la covariance comme
$$\Sigma_{U\vert V} = (I_{d_U} - KA)\Sigma_U$$
où $K$ est une matrice $d_U \times d_V$: 
$$K = \Sigma_UA^{'}(A\Sigma_UA^{'}+\Sigma_{V\vert U})^{-1}$$
Ensuite, en remarquant
\footnote{En effet, ceci est équivalent à:
$$\Sigma_U A\Sigma_{V\vert U}^{-1} = K(Id_V + A\Sigma_UA^{'}\Sigma_{V\vert U}^{-1}).$$
Cette dernière égalité en factorisant par $\Sigma_{V\vert U}^{-1})$ à droite et en utilisant la définition de $K$.} 
que:
$$
K = (I_{d_U} - KA)\Sigma_UA^{'}\Sigma_{V\vert U}^{-1}
$$
on peut réécrire \eqref{eq:loi:U:cond:V}
\begin{align}
U \vert \left\lbrace V = v \right\rbrace &\sim \mathcal{N}\left(m_U + K(v - (Am_U + b)), (I_{d_U} - K A)\Sigma_U \right) \label{eq:loi:U:cond:V:alter},
\end{align}

On remarquera que les équations \eqref{eq:loi:U:cond:V} et \eqref{eq:loi:V} correspondent aux densités:
\begin{align}
p(u\vert v) \propto p(u)p(v \vert u) \label{eq:dens:U:cond:V}\\
p(v) = \int p(v\vert u) p(u) \text{d}u \label{eq:dens:V}
\end{align}


## Loi de filtrage

Comme quantité essentielle apparaissant dans l'équation de lissage vue plus haut est la quantité $p(x_k \vert y_{0:k}), (0\leq k \leq n)$.
Les deux propriétés précédentes nous montrent que cette densité est celle d'une loi Gaussienne:

Pour le montrer, raisonnons par récurrence:

**Cas $k = 0$**

D'après \eqref{eq:loi:U:cond:V}:
$$X_0 \vert \left\lbrace Y_0 = y_0\right\rbrace \sim \mathcal{N}(m_{0\vert 0}, \Sigma_{0\vert 0})$$
où $m_{0\vert 0}$ et $\Sigma_{0 \vert 0}$ désignent la moyenne et la variance de la loi, donnée par l'équation \eqref{eq:loi:U:cond:V}.

**Cas k > 0** Supposons que la proposition est vraie pour $k-1$.
On remarque que:
\begin{align*}
p(x_k\vert y_{0:k}) &= \int p(x_k, x_{k - 1}\vert y_{0:k})\text{d}x_{k - 1}\\
&\propto \int p(y_{0:k}, x_k, x_{k - 1})\text{d}x_{k - 1}\\
&\propto \int p(y_{k}\vert x_k, x_{k - 1}, y_{0:k-1})p(x_{k}\vert x_{k - 1}, y_{0:k-1})p(x_{k - 1}\vert y_{0:k-1})\text{d}x_{k - 1}\\
&\propto \int p(y_{k}\vert x_k)p(x_{k}\vert x_{k - 1})p(x_{k - 1}\vert y_{0:k-1})\text{d}x_{k - 1}\\
&\propto p(y_{k}\vert x_k) \int p(x_{k}\vert x_{k - 1})p(x_{k - 1}\vert y_{0:k-1})\text{d}x_{k - 1}
\end{align*}

On peut ici identifier les différents termes à ceux de la proposition précédente.
L'identification est la suivante:
\begin{align*}
U &= \left\lbrace X_{k-1} \vert Y_{0:k-1}\right\rbrace\\
V &= \left\lbrace X_{k} \vert Y_{0:k-1}\right\rbrace\\
m_U &= m_{k - 1\vert k - 1}\\
\Sigma_U &= \Sigma_{k - 1\vert k - 1}\\
m_{V\vert \left\lbrace U = u \right\rbrace} &= \mu_k + A_{k}u\\
\Sigma_{V\vert \left\lbrace U = u \right\rbrace} &= B_kB_k'
\end{align*}
Donc le résultat de l'intégrale est la densité d'une loi 
$$
\mathcal{N}\left(A_{k}
m_{k - 1\vert k - 1} + \mu_k,
\underbrace{B_kB_k' + A_k \Sigma_{k - 1\vert k - 1} A_k'}_{\Gamma_k}
\right)
$$
Ensuite, la deuxième identification peut se faire 
\begin{align*}
U &= \left\lbrace X_{k} \vert Y_{0:k-1}\right\rbrace\\
V &= \left\lbrace Y_{k} \vert Y_{0:k-1}\right\rbrace\\
m_U &= A_km_{k - 1\vert k - 1} + \mu_k\\
\Sigma_U &= B_kB_k' + A_k \Sigma_{k - 1\vert k - 1} A_k'\\
m_{V\vert \left\lbrace U = u \right\rbrace} &= C_{k}u\\
\Sigma_{V\vert \left\lbrace U = u \right\rbrace} &= D_kD_k'
\end{align*}

L'équation \label{eq:loi:U:cond:V:alter} donne donc
$$
\mathcal{N}
\left(
A_km_{k - 1\vert k - 1} + \mu_k + K_k(y_k - C_k(A_km_{k - 1\vert k - 1} + \mu_k)),
(I-K_kC_k)\Gamma_k
\right)
$$
où $K_k$ est le gain de Kalman
$$
K_k = \Gamma_k C_k^{'} (D_kD_k^{'}+ C_k\Gamma_kC_k^{'})^{-1}
$$

Ainsi, pour tout $k$, la loi de $X_k\vert Y_{0:k}$ est Gaussienne, et ses moments se calculent récursivement, dans une passe avant.

En particulier, on obtient la loi de $X_n\vert Y_{0:n}$ qui est une loi Gaussienne de moments connus.

## Loi de lissage

Si on en revient à la loi de lissage, pour $k = n$,
on a une loi normale de moyenne $m_{n \vert n}$ et de variance $\Sigma_{n\vert n}$.
Pour $0\leq k \leq n - 1$, on a:

\begin{align*}
p(x_k \vert y_{0:n}) &\propto  \int p(x_{k + 1}\vert x_{k}) p(x_k \vert  y_{0:k}) p(x_{k + 1} \vert y_{0:n}) \text{d} x_{k + 1}
\end{align*}

Dans l'intégrale, le premier produit peut être identifié comme la loi de $U\vert V$ avec
L'identification est la suivante:
\begin{align*}
U &= \left\lbrace X_{k} \vert Y_{0:k}\right\rbrace\\
V &= \left\lbrace X_{k + 1} \vert Y_{0:k}\right\rbrace\\
m_U &= m_{k\vert k}\\
\Sigma_U &= \Sigma_{k\vert k}\\
m_{V\vert \left\lbrace U = u \right\rbrace} &= \mu_{k + 1} + A_{k + 1}u\\
\Sigma_{V\vert \left\lbrace U = u \right\rbrace} &= B_{k+1}B_{k +1}^{'}
\end{align*}

Ainsi, on a:
$$
X_k \vert \left\lbrace X_{k + 1} = x_{k+1}, Y_{0:k}\right\rbrace 
\sim
\mathcal{N}
\left(
\tilde{K}_{k+1}x_{k + 1} + m_{k\vert k}   - \tilde{K}_{k+1}(A_{k + 1} m_{k\vert k} + \mu_{k+1})),
(I - \tilde{K}_{k+1} A_{k + 1})\Sigma_{k\vert k}
\right)
$$

où:
$$\tilde{K}_{k+1} = \Sigma_{k\vert k}A_{k+1}^{'}(A_{k+1}\Sigma_{k\vert k}A_{k+1}^{'} + B_{k + 1}B_{k + 1}^{'} )$$

Ainsi, par conjugaison, l'intégrale du produit résultant donne:
$$
X_{k} \vert \left\lbrace Y_{0:n}\right\rbrace 
\sim
\mathcal{N}
\left(
m_{k\vert k}   - \tilde{K}_{k+1}(m_{k+1\vert n} + A_{k + 1} m_{k\vert k} + \mu_{k+1})),
(I - \tilde{K}_{k+1} A_{k + 1})\Sigma_{k\vert k} + \tilde{K}_{k+1}\Sigma_{k + 1\vert n}\tilde{K}_{k+1}^{'}
\right)
$$



# Annexe: Processus de Ornstein Uhlenbeck

Soit $\left\lbrace V_t \right \rbrace_{t \geq 0}$ un processus stochastique à valeurs dans $\mathbb{R}$ solution de l'équation différentielle stochastique:

$$\text{d}V_t = -\rho\left(X_t - \mu \right) + \sigma \text{d}W_t,~X_0 \sim \chi_0$$

On définit alors $\left\lbrace V_t \right \rbrace_{t \geq 0}$ de la manière suivante:
$$Z_t = Z_0 + \int_0^t V_s\text{d}s$$

On a alors les propriétés suivantes:
\begin{align}
V_{t + \Delta}\vert \left\lbrace V_t = v_t \right\rbrace &\sim \mathcal{N}\left(\text{e}^{-\rho\Delta}v_t + (1 - \text{e}^{-\rho\Delta})\mu , \frac{\sigma^2}{2\rho}(1 - \text{e}^{-2\rho\Delta}) \right)\\
Z_{t + \Delta} \vert \left\lbrace V_t = v_, Z_t = z_t\right \rbrace &\sim \mathcal{N}\left(x_0 + \mu t +  \frac{1 - \text{e}^{-\rho t}}{\rho}(v_0 - \mu), 
\frac{\sigma^2}{\rho^2}\left(t + \frac{1}{\rho}\left(- \frac{\text{e}^{-2\rho\Delta}}{2} + 2\text{e}^{-\rho\Delta} - \frac{3}{2} \right)\right) \right).
\end{align}
De plus, on a:
\begin{align}
\mathbb{C}ov[V_{t + \Delta}, Z_{t + \Delta}\vert \left\lbrace V_t = v_, Z_t = z_t\right] &= \frac{\sigma^2}{2\rho^2}\left(1 -  \text{e}^{-2\rho\Delta} \right)^2
\end{align}